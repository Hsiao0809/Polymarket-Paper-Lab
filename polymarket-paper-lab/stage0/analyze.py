...
def report(rows: list[dict]) -> str:
    ...
    if pct < 5:
        verdict = ("結論：吃單腿基本上死了。\n"
                   ...
                   "  → 若要繼續，只能走做市（maker）方向：零手續費 + 費用回饋。\n"
                   "    但注意獎勵池規模浮動且可能為零，見下方做市側觀測。")
    elif pct < 20:
    ...
    out.append("做市側參考")
    out.append("=" * 62)
    out.append(f"\nMaker 手續費         0（僅 taker 付費）")
    out.append(f"Taker 費回饋 maker   {config.FEE_REBATE_RATE:.0%}")

    # 獎勵率會劇烈變動，必須看分布而非單一數值
    rds = [r["rewards_daily"] for r in ok if r["rewards_daily"] is not None]
    n_null = len(ok) - len(rds)
    out.append(f"\n每日獎勵池觀測       {len(rds)} 筆有值 / {n_null} 筆為 null")
    if rds:
        srt = sorted(rds)
        out.append(f"  最小 / 中位 / 最大  ${srt[0]:,.0f} / ${srt[len(srt)//2]:,.0f} / ${srt[-1]:,.0f}")
        if srt[-1] > 0 and srt[0] / max(srt[-1], 1e-9) < 0.1:
            out.append("  ⚠ 獎勵率變動超過一個數量級——不可用單一數值做收益推估。")
    else:
        out.append("  ⚠ 完全沒有觀測到獎勵率。做市側收益無法評估。")

    minsz = {r["rewards_minsz"] for r in ok if r["rewards_minsz"] is not None}
    if minsz:
        out.append(f"獎勵最小掛單量觀測值  {sorted(minsz)}")

    out.append("\n注意：獎勵池由所有合格 maker 按比例分配。"
               "\n      你的份額 ≈ 你的合格掛單量 / 全體合格掛單量。"
               "\n      這無法從公開盤口精確反推，需實際掛單後觀察。"
               "\n      且獎勵是平台補貼，隨時可調降或撤除——實測已見過中途消失。")
